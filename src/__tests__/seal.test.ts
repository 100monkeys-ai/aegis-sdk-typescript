...
describe('Ed25519Key', () => {
  ...
  test('getPublicKeyBase64 round-trips through base64', () => {
    ...
    expect(Buffer.from(b64, 'base64')).toEqual(Buffer.from(key.getPublicKeyBytes()));
  });
...
describe('createCanonicalMessage', () => {
  ...
  test('top-level keys are sorted', () => {
    const msg = new TextDecoder().decode(createCanonicalMessage('tok', payload, 100));
    // payload < security_token < timestamp
    expect(msg.indexOf('"payload"')).toBeLessThan(msg.indexOf('"security_token"'));
    expect(msg.indexOf('"security_token"')).toBeLessThan(msg.indexOf('"timestamp"'));
  });

  test('contains no whitespace', () => {
    const msg = new TextDecoder().decode(createCanonicalMessage('tok', payload, 1));
...
describe('createSealEnvelope', () => {
  test('returns correct structure', () => {
    const key = Ed25519Key.generate();
    const payload: McpPayload = { jsonrpc: '2.0', id: 'r1', method: 'tools/call' };
...
function makeValidEnvelope(): { envelope: unknown; publicKeyBytes: Uint8Array } {
  const key = Ed25519Key.generate();
...
describe('verifySealEnvelope', () => {
  ...
  test('throws on wrong protocol', () => {
    ...
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow(SEALError);
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('protocol');
  });

  test('throws on missing security_token', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    delete (envelope as Record<string, unknown>)['security_token'];
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('security_token');
  });

  test('throws on missing signature', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    delete (envelope as Record<string, unknown>)['signature'];
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('signature');
  });

  test('throws on missing payload', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    delete (envelope as Record<string, unknown>)['payload'];
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('payload');
  });

  test('throws on missing timestamp', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    delete (envelope as Record<string, unknown>)['timestamp'];
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('timestamp');
  });

  test('throws on expired envelope', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    (envelope as Record<string, unknown>)['timestamp'] = '2020-01-01T00:00:00.000Z';
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('window');
  });

  test('throws on invalid timestamp format', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    (envelope as Record<string, unknown>)['timestamp'] = 'not-a-date';
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('ISO 8601');
  });

  test('throws on bad signature', () => {
    const { envelope, publicKeyBytes } = makeValidEnvelope();
    (envelope as Record<string, unknown>)['signature'] = Buffer.alloc(64, 0xff).toString('base64');
    expect(() => verifySealEnvelope(envelope, publicKeyBytes)).toThrow('signature verification failed');
  });

  test('throws on non-object input', () => {
    expect(() => verifySealEnvelope(null, new Uint8Array(32))).toThrow(SEALError);
    expect(() => verifySealEnvelope('string', new Uint8Array(32))).toThrow(SEALError);
  });
});
...
describe('SEALClient', () => {
  test('constructor strips trailing slash', () => {
    const client = new SEALClient('http://gateway.example.com/', 'wl-1', 'read-only');
  ...
  describe('attest', () => {
    ...
    test('returns AttestationResult on success', async () => {
      ...
      const client = new SEALClient('http://gw.example.com', 'wl-1', 'read-only');
    ...
    test('throws SEALError on error status', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        json: async () => ({ status: 'error', message: 'Unrecognised workload' }),
      });
  ...
  describe('callTool', () => {
    ...
    function makeAttestedClient(): SEALClient {
      const client = new SEALClient('http://gw.example.com', 'wl-1', 'code-assistant');
      // Inject internal state directly (testing internals via type cast)
      (client as unknown as Record<string, unknown>)['securityToken'] = 'eyJtok';
      (client as unknown as Record<string, unknown>)['key'] = Ed25519Key.generate();
      return client;
    }
    ...
    test('returns result on success', async () => {
      ...
      const result = await client.callTool('fs.read', { path: '/tmp/test.txt' });
    ...
    test('throws SEALError when gateway rejects', async () => {
      ...
      const client = makeAttestedClient();
      await expect(client.callTool('fs.write', {})).rejects.toThrow('SEAL Gateway Rejected');
    });
    ...
    test('throws SEALError on gateway error in response body', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ status: 'error', error: { message: 'Internal error' } }),
      });

      const client = makeAttestedClient();
      await expect(client.callTool('fs.read', {})).rejects.toThrow('SEAL Gateway Error');
    });
    ...
    test('throws SEALError on MCP payload error', async () => {
      ...
      const client = makeAttestedClient();
      await expect(client.callTool('fs.read', {})).rejects.toThrow('MCP Tool Error');
    });
